...
function inviteUrl(invitationCode: string): string {
  if (typeof window === 'undefined') return '';
  return `${window.location.origin}${window.location.pathname}#invite/${invitationCode}`;
}
...
export const InvitationsTabContent: React.FC<InvitationsTabContentProps> = ({
  ...
  const handleCopyLink = async (inv: OwnerInvitationView) => {
    const url = inviteUrl(inv.invitation_code);
