...
from app.services.permissions import owner_profile_property_ids, user_owns_property_by_profile
...
def list_stays(
    ...
    if as_guest:
        ...
        stays = (
            db.query(Stay).filter(Stay.property_id.in_(owned_ids)).all()
            if owned_ids
            else []
        )
...
def get_stay(
  ...
  current_user: User = Depends(get_current_user),
    ...
    if stay.guest_id == current_user.id:
        return StayResponse.model_validate(stay)
    if user_owns_property_by_profile(db, current_user.id, stay.property_id):
        return StayResponse.model_validate(stay)
