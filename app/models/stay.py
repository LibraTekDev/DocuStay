...
class Stay(Base):
    ...
    checked_in_at = Column(DateTime(timezone=True), nullable=True)  # when guest clicks Check in; occupancy / Status Confirmation apply only after this
    checked_out_at = Column(DateTime(timezone=True), nullable=True)
    cancelled_at = Column(DateTime(timezone=True), nullable=True)

    # Stay end reminders (copied from invitation when stay is created)
    ...
    # Confirmation flow: when Status Confirmation prompts, owner must select Unit Vacated | Lease Renewed | Holdover before deadline
